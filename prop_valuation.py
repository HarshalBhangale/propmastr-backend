...
def geolocate(address, api_key):
    ...
    try:
        ...
        raise Exception(f"Geolocation error for address '{address}': {e}")
...
def filter_results(address, property_type, industrial_size, industrial_subtype, office_grade, distance=3000, expense=0.3):
    location = geolocate(address, os.getenv('GOOGLE_MAPS_API_KEY'))
    if not location:
        raise Exception("Location not found for the given address.")
    ...
    if caprates.empty:
        raise Exception("Cap rates not found near the given location.")
